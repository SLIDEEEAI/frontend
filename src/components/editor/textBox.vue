<template>
  ...
  <contentBox
    ...
    :is-loading="editable && textBoxObject.loading"
  ...
  >
    ...
    <div class="placeholder" v-if="textBoxObject.text == '' && showPlaceholder">Введите текст...</div>
      ...
      <!-- v-select -->
        <div 
          v-for="(paragraph, index) in textContentDivs" 
          :key="index"
...
const updateStyleState = (newState : State) => {
  ...
  styleState.color = newState.color 
  // с этим после скрытия эдит панели свойство присваивается текстбоксу и больше не наследуется
  // || colorByFontTheme()
